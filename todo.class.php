            ...
            while ($row=$stmt->fetch(PDO::FETCH_ASSOC)){
                ...
                <div class="col-md-4 text-left mt-2">
                    ...
                    <form method="post">
                        <?php
                        if ($row['completed'] == 1){
                            echo "<input type='radio' class='custom-control-input' name=rad-{$row['id']} value='1' checked /> Completed task <br/>";
                            echo "<input type='radio'  class='custom-control-input' name=rad-{$row['id']} value='0' /> An incomplete task <br/>";
                            ...
                            echo "<button class='btn btn-primary' type='submit' value={$row['id']} class='' name='btn-complete'> Complete </button>";
                        }
                        ?>
                        <button class='btn btn-info'  type="submit" value= <?php print($row['id']); ?> class="" name="btn-edit"> Edit </button>
                        <button onclick="return confirm('Are you sure you want to delete this item?');" class='btn btn-danger'  type="submit" value= <?php print($row['id']); ?> class="" name="btn-delete"> Delete </button>
                    </form>
                </div>
            <?php
            }
